...
1. **Arc clearance**
   ...
   * For every numeric angle with an internal label:
     `dist(label, arc) ≥ δ_clear` and `r_lbl ≤ r_arc + cap`.

2. **Narrow wedge externalization**

   * If `ω < 12°`, numeric labels are **external** and include a leader.

3. **Equal‑angle stack respect**

   * With a group of `g` equal‑angle arcs at a vertex and one numeric angle: numeric arc radius is `r0 + g·\gsAngSep`; label sits outside it (internal or external).

4. **No global radius inflation**

   * Changing the label from internal→external **does not** change the drawn arc radius (visual comparison or assert on emitted `angle radius`).
...
* Keep units consistent: `\gsAngSep` and `\gsAngR` are TeX lengths (pt). The algorithm uses only **dimensionless ratios** (`ecc`) and **multiples of `\gsAngSep`**, so no world‑unit conversion is required.
* `pt_per_em=9.0` is a stable approximation for `\footnotesize` (only used to estimate how far we must move the label off the arc).
* The collision probe in Step C can reuse the **same segment/circle proximity logic** used for point labels (§20.1) but restricted to a small neighborhood around the candidate.
* Right angles are unaffected (still square marks only). Equal‑angle arcs remain label‑free and sit **inside** any numeric arc per §19.13.
